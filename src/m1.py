...
"""  # ODO: 1. PUT YOUR NAME IN THE ABOVE LIN.
...
class CircleChanger(object):
    ...
    def swallow(self, other_circle_changer):
        ...
        Type hints:
            ...
            :rtype CircleChanger
        """
        radius = .5 * self.circle.center.get_distance_from(other_circle_changer.circle.center)
    ...
    def change_to_next_color_in_tuple(self):
        ...
        """
        if (self.colorcalls == 0) or (self.colorcalls == len(self.colors)):
            self.circle.fill_color = self.colors[0]
            self.colorcalls = 1
        else:
            self.colorcalls = self.colorcalls + 1
            self.circle.fill_color = self.colors[self.colorcalls - 1]
